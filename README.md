...
First, clone the repository recursively to fetch DPF and all necessary module submodules:
```bash
...
- **Arch Linux**:
  ```bash
  sudo pacman -S cmake dbus file fftw libgl liblo libsndfile libx11 libxcursor libxext libxrandr python3 wget
  ```

##### 3. Build for Linux (General Desktop / x86_64)
To build Cardinal for your standard desktop Linux system:
```bash
make
```
This builds all plugins and standalone variants under the `bin/` directory.

##### 4. Build for Raspberry Pi 5 (running Raspbian / Raspberry Pi OS)
To build specifically for the Raspberry Pi 5:
- **Native compilation** (on the Pi itself):
  ```bash
  make RPI5=true
  ```
- **Cross-compilation** (from a 64-bit Linux host):
  ```bash
  ...
  make RPI5=true
  ```
The `RPI5=true` flag automatically optimizes compiler flags for Cortex-A76 and configures the UI to use OpenGL ES 2.0 (`gles2`) for optimal performance on the Broadcom VideoCore VII GPU.
