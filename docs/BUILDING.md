...
When building natively or cross-compiling for a Raspberry Pi 5 running Raspberry Pi OS (Raspbian), you can use the `RPI5=true` option. This will:
1. Optimize the compiler flags specifically for the Raspberry Pi 5's Cortex-A76 cores (`-mcpu=cortex-a76`).
2. Default the graphics subsystem to OpenGL ES 2.0 (`UI_TYPE=gles2`), which is the recommended and high-performance option for the VideoCore VII GPU on Raspberry Pi.

To build, install the standard Debian/Ubuntu dependencies (see the Debian section below) and run:

```bash
make RPI5=true
```

Or, if you are cross-compiling from a 64-bit Linux host, target it like so:
