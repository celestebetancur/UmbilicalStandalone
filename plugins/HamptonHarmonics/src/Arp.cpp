...
void Arp::process(const ProcessArgs& args) {
	...
	bool isAnyGateHigh = inputs[GATE_INPUT].getVoltageSum(); // If getVoltageSum returns anything other than 0, that means at least one channel's gate is high
	...
	for (int c = 0; c < numberOfChannels; c++) {
		// if this channel has a high gate, then we add the corresponding pitch to the list of pitches
		if (inputs[GATE_INPUT].getPolyVoltage(c)) {
	...
	// Reset
	if (this->_resetTrigger.process(inputs[RESET_INPUT].getVoltage()) || (!isAnyGateHigh && !this->_shouldLatch)) {
		setStep(-1, numberOfPitches);
		setPolyChannel(-1);
	}

	// Handle latching
	if (this->_shouldLatch) {
		if (this->_isNewGate && isAnyGateHigh) {
	...
	// Step if clock is high and a gate is high (or is latched)
	if (isClockHigh && (isAnyGateHigh || this->_shouldLatch)) {
	...
	// Set the pitch out
	// We could just always set pitch out, but that takes unnecessary computation so let's just do it when we need it
	...
	if (this->_isPolyOutput) {
		...
		outputs[GATE_OUTPUT].setVoltage(isAnyGateHigh || this->_shouldLatch ? this->_gateGenerator.process(this->_sampleTime) * 5.0f : 0.0f, this->_polyChannel);
	} else {
		outputs[PITCH_OUTPUT].setChannels(1);
		outputs[GATE_OUTPUT].setChannels(1);
		outputs[PITCH_OUTPUT].setVoltage(this->_pitchOut);
		outputs[GATE_OUTPUT].setVoltage(isAnyGateHigh || this->_shouldLatch ? this->_gateGenerator.process(this->_sampleTime) * 5.0f : 0.0f);
	}
